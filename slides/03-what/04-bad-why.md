## Pourquoi adopter cette pratique ? .[steps]

- Tracer toutes les décisions pour la postérité,
- Justifier ses choix auprès des architectes ou de la hiérarchie,
- Imposer une décision de façon descendante (le fameux Contre-Kem's),
- Mesurer la maturité d'architecture d'une équipe,
- Pouvoir auditer les équipes et leurs choix d'architecture,
- Valider les choix techniques avant leur mise en œuvre,
- Créer une source unique de vérité,
- Faciliter l'onboarding des nouveaux arrivants.

/*

<<Logan Déroule Slides>>

{Logan} Je suis sûr que vous avez déjà entendu une ou plusieurs de ces bonnes raisons.

{Logan} Mais finalement, elles nous font dire que les ADRs :
- c'est bureaucratique,
- c'est pénible, on abandonne vite,
- on préfère rush le truc,
- impression d'être fliqué,
- personne ne les relit (sauf au moment des audits d'archi, mais là il est trop tard),
- on rédige tout seul, en autonomie, c'est franchement pas stimulant,
- c'est devenu un objectif en soit (cf. Loi de Goodhart),
- bref ça donne pas envie quoi,

{Nicolas} Et c'est en ça qu'on peut comprendre que les équipes n'adhèrent pas à la pratique ADR dans la durée.

🐒 🐵 🙈 🙉 🙊

*/